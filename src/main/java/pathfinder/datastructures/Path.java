 ...
 */
public class Path<Node> implements Iterable<Path<Node>.Segment<Node>> {
     ...
     */
    private List<Segment<Node>> path;
     ...
     */
    @Override
    public Iterator<Segment<Node>> iterator() {
        // Create a wrapping iterator to guarantee exceptional behavior on Iterator#remove.
        return new Iterator<Segment<Node>>() {

            private Iterator<Segment<Node>> backingIterator = path.iterator();
            ...
            @Override
            public Path<Node>.Segment<Node> next() {
